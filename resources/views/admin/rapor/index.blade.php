    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form method="GET" class="mt-6 flex flex-wrap items-end gap-3">
            <div class="space-y-2">
                ...
                <select id="semester" name="semester" class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="tahun_ajaran" name="tahun_ajaran" value="{{ $tahunAjaran }}" placeholder="2025/2026"
                    class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm">
            </div>
            <div class="space-y-2">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($rapor as $item)
                            <tr>
                                <td class="px-5 py-4 font-semibold text-slate-900">{{ $item->student->user?->name ?? $item->student->name }}</td>
                                <td class="px-5 py-4 text-slate-600">{{ $item->counselor?->name ?? '-' }}</td>
                                <td class="px-5 py-4 text-slate-600">{{ $item->semesterLabel() }} · {{ $item->tahun_ajaran }}</td>
                                <td class="px-5 py-4">{{ $item->statusLabel() }}</td>
                                <td class="px-5 py-4 text-right">
                                    <a href="{{ route('admin.rapor.show', $item) }}" class="rounded-2xl border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
