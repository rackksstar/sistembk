...
<div class="space-y-6" x-data="{ createOpen: {{ $errors->any() ? 'true' : 'false' }}, editOpen: null }">
    <section class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
        ...
        <form method="GET" action="{{ route('admin.kelas.index') }}" class="mt-6 grid gap-3 lg:grid-cols-[minmax(0,1fr)_260px_180px_auto]">
            ...
            <select name="jenjang" class="rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800 bg-white dark:bg-slate-900">
                        @forelse($kelas as $item)
                            <tr>
                                ...
                                <td class="px-5 py-4 text-slate-600 dark:text-slate-400">{{ $item->jenjang ?? '-' }}</td>
                                <td class="px-5 py-4 text-slate-600 dark:text-slate-400">{{ $item->tingkatan ?? '-' }}</td>
                                <td class="px-5 py-4">
                            ...
                            <tr>
                                <td colspan="5" class="px-5 py-6">
                                    ...
                                    <div class="mt-4 text-center">
                                        <button type="button" x-on:click="createOpen = true" class="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-semibold text-white hover:bg-blue-500">Tambah kelas</button>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
