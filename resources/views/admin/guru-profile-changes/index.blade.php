                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($changes as $change)
                            <tr>
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-slate-900">{{ $change->user?->name ?? '-' }}</p>
                                    <p class="text-xs text-slate-500">{{ $change->user?->guruBkProfile?->sekolah?->nama ?? $change->user?->school ?? '-' }}</p>
                                </td>
                                ...
                                <td class="px-5 py-4 text-slate-600">{{ $change->created_at?->format('d M Y H:i') }}</td>
                                <td class="px-5 py-4">
                                    <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] ring-1 {{ $change->reviewed_at ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-amber-50 text-amber-700 ring-amber-200' }}">
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex justify-end">
                                        ...
                                        @else
                                            <span class="text-xs text-slate-500">{{ $change->reviewed_at->format('d M Y H:i') }}</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
