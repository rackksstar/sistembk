...
<select name="kelas_id" required class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
...
<select name="jenis_kelamin" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">
...
<textarea name="alamat" rows="2" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" placeholder="Alamat (opsional)">{{ old('alamat', $student?->alamat) }}</textarea>
<x-input-error :messages="$errors->get('alamat')" class="text-sm text-red-600" />

@if($student)
    <p class="text-xs text-slate-500">Status biodata: <span class="font-semibold">{{ $student->status_biodata === 'lengkap' ? 'Lengkap' : 'Belum lengkap' }}</span> (otomatis dari jenis kelamin + alamat)</p>
@endif

<input name="school" value="{{ old('school', $student?->school) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm" placeholder="Sekolah (opsional, legacy)" />
