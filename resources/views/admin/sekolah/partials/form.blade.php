...
<div class="grid gap-4 sm:grid-cols-2">
    <div class="space-y-2">
        <label class="block text-sm font-semibold text-slate-900" for="npsn-{{ $sekolah?->id ?? 'create' }}">NPSN</label>
        <input id="npsn-{{ $sekolah?->id ?? 'create' }}" name="npsn" value="{{ old('npsn', $sekolah?->npsn) }}" required class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100" placeholder="NPSN sekolah" />
        <x-input-error :messages="$errors->get('npsn')" class="text-sm text-red-600" />
    </div>
    <div class="space-y-2">
        <label class="block text-sm font-semibold text-slate-900" for="logo-{{ $sekolah?->id ?? 'create' }}">Logo (opsional)</label>
        <input id="logo-{{ $sekolah?->id ?? 'create' }}" name="logo" type="file" accept="image/*" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100" />
        <x-input-error :messages="$errors->get('logo')" class="text-sm text-red-600" />
    </div>
</div>

<div class="space-y-2">
    <label class="block text-sm font-semibold text-slate-900" for="alamat-{{ $sekolah?->id ?? 'create' }}">Alamat Sekolah</label>
    <textarea id="alamat-{{ $sekolah?->id ?? 'create' }}" name="alamat" required rows="3" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100" placeholder="Alamat lengkap sekolah">{{ old('alamat', $sekolah?->alamat) }}</textarea>
...
<div class="grid gap-4 sm:grid-cols-2">
    <div class="space-y-2">
        <label class="block text-sm font-semibold text-slate-900" for="mou-{{ $sekolah?->id ?? 'create' }}">Status MOU</label>
        <select id="mou-{{ $sekolah?->id ?? 'create' }}" name="is_mou" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100">
