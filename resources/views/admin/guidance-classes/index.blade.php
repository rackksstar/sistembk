...
<div class="space-y-6" x-data="{ createOpen: false, editOpen: null }">
            ...
            <article class="rounded-3xl border border-blue-100 dark:border-blue-900/50 bg-white dark:bg-slate-900 p-6 shadow-sm">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 inline-flex rounded-full bg-blue-50 dark:bg-blue-950/40 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-blue-700 dark:text-blue-300">Kode: {{ $class->code }}</p>
                        <p class="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{{ $class->description ?? 'Belum ada deskripsi.' }}</p>
                    </div>
                    <div class="flex gap-2">
                        <button x-on:click="editOpen = {{ $class->id }}" class="rounded-2xl border border-slate-200 dark:border-slate-700 px-3 py-2 text-xs font-semibold">Edit</button>
                ...
                <form method="POST" action="{{ route('admin.guidance-classes.students.attach', $class) }}" class="mt-5 flex flex-col gap-3 sm:flex-row">
                    @csrf
                    <select name="student_id" required class="min-w-0 flex-1 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm">
                ...
                <div class="mt-5 space-y-2">
                    @forelse($class->students as $student)
                        <div class="flex items-center justify-between gap-3 rounded-2xl bg-blue-50 dark:bg-blue-950/40 px-4 py-3">
                            <div>
                                <p class="text-sm font-semibold text-slate-900 dark:text-slate-100">{{ $student->name }}</p>
                                <p class="text-xs text-slate-500 dark:text-slate-400">NISN {{ $student->nisn }}</p>
                            </div>
    ...
    <div x-show="createOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
        <div x-on:click.outside="createOpen = false" class="w-full max-w-lg rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-2xl">
    ...
    @foreach($classes as $class)
        <div x-show="editOpen === {{ $class->id }}" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
            <div x-on:click.outside="editOpen = null" class="w-full max-w-lg rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-2xl">
