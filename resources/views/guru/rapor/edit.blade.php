        ...
        <form method="POST" action="{{ route('guru.rapor.update', $student) }}" class="mt-6 space-y-5">
            ...
            <div class="space-y-2">
                ...
                <textarea id="perkembangan_akademik" name="perkembangan_akademik" rows="4"
                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('perkembangan_akademik', $rapor?->perkembangan_akademik) }}</textarea>
            ...
            <div class="space-y-2">
                ...
                <textarea id="perkembangan_sosial" name="perkembangan_sosial" rows="4"
                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('perkembangan_sosial', $rapor?->perkembangan_sosial) }}</textarea>
            ...
            <div class="space-y-2">
                ...
                <textarea id="perkembangan_psikologis" name="perkembangan_psikologis" rows="4"
                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('perkembangan_psikologis', $rapor?->perkembangan_psikologis) }}</textarea>
            ...
            <div class="space-y-2">
                ...
                <textarea id="saran_tindak_lanjut" name="saran_tindak_lanjut" rows="3"
                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('saran_tindak_lanjut', $rapor?->saran_tindak_lanjut) }}</textarea>
            ...
            <div class="space-y-2">
                ...
                <textarea id="catatan_guru" name="catatan_guru" rows="2"
                    class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm">{{ old('catatan_guru', $rapor?->catatan_guru) }}</textarea>
                <x-input-error :messages="$errors->get('catatan_guru')" />
            </div>

            <div class="space-y-2">
                <x-input-label for="status" value="Status rapor" />
                <select id="status" name="status" class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                @if($rapor)
                    <a href="{{ route('guru.rapor.pdf', $rapor) }}"
                        class="rounded-2xl border border-blue-200 bg-blue-50 px-5 py-2.5 text-sm font-semibold text-blue-700 hover:bg-blue-100">
                        Unduh PDF
                    </a>
                @endif
            </div>
