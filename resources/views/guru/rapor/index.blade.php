        ...
        <form method="GET" class="mt-6 flex flex-wrap items-end gap-3">
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="tahun_ajaran" name="tahun_ajaran" value="{{ $tahunAjaran }}" placeholder="2025/2026"
                    class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm" pattern="\d{4}/\d{4}">
            </div>
            <div class="space-y-2">
            ...
            <button class="rounded-2xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white">Terapkan</button>
        </form>
    </section>
                        ...
                        @forelse($students as $student)
                            @php
                                $statusClass = match ($student->rapor_periode?->status) {
                                    \App\Models\RaporBk::STATUS_FINAL => 'bg-emerald-100 text-emerald-800',
                                    \App\Models\RaporBk::STATUS_DRAFT => 'bg-amber-100 text-amber-800',
