            ...
            <article class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h3 class="mt-2 text-lg font-bold text-slate-950">{{ $submission->student?->name }}</h3>
                        <p class="text-sm text-slate-500">{{ $submission->submitted_at?->format('d M Y H:i') }}</p>
                ...
                <div class="mt-4 rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <p class="font-semibold text-slate-900">{{ $submission->result_label }}</p>
                    <p class="mt-1 text-sm leading-6 text-slate-600">{{ $submission->result_description }}</p>
                </div>
            </article>
        @empty
