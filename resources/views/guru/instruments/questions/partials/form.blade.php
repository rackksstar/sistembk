...
<div class="space-y-4">
    ...
    <div class="space-y-2">
        ...
        <textarea id="question_{{ $question?->id ?? 'new' }}" name="question" rows="4" required class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100">{{ old('question', $question?->question) }}</textarea>
    </div>

    <div class="space-y-3">
        <p class="text-sm font-semibold text-slate-900">Pilihan Jawaban dan Skor</p>
    ...
    <label class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700">
        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $question?->is_active ?? true)) class="rounded border-slate-300 text-blue-600 focus:ring-blue-500">
