...
<div class="space-y-6" x-data="{ detailOpen: null, scheduleOpen: null, reportOpen: null, rejectOpen: null }">
    <section class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
        ...
        <div id="consultation-calendar" class="mt-6 min-h-[420px] rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 p-2"></div>
                                ...
                                <td class="px-5 py-4">
                                    <div class="flex flex-wrap justify-end gap-2">
                                        ...
                                        @if($consultation->result)
                                            <a href="{{ route('guru.consultations.print', $consultation) }}" target="_blank" class="rounded-2xl bg-white dark:bg-slate-900 px-3 py-2 text-xs font-semibold text-blue-700 dark:text-blue-300 ring-1 ring-blue-100">PDF</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
        ...
        <div x-show="rejectOpen === {{ $consultation->id }}" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
            <div x-on:click.outside="rejectOpen = null" class="w-full max-w-lg rounded-3xl bg-white dark:bg-slate-900 p-6 shadow-2xl">
                ...
                <form method="POST" action="{{ route('guru.consultations.reject', $consultation) }}" class="mt-6 space-y-4">
                    @csrf @method('PATCH')
                    <textarea name="rejection_reason" rows="4" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm" placeholder="Alasan penolakan"></textarea>
