<x-guest-layout role="guru">
    <div class="space-y-8">
        <div class="space-y-3 text-center">
            ...
            <h1 class="text-3xl font-semibold text-slate-950 dark:text-white sm:text-4xl">Ajukan akun Guru BK</h1>
            <p class="text-sm leading-6 text-slate-600 dark:text-slate-400">Hanya sekolah yang sudah MOU dengan PCR yang bisa mengajukan akun.</p>
        </div>
        ...
        <form method="POST" action="{{ route('guru.register.store') }}" class="space-y-5">
            ...
            <div class="space-y-2">
                <label for="name" class="block text-sm font-semibold text-slate-900 dark:text-slate-100">Nama</label>
                <input id="name" name="name" value="{{ old('name') }}" required autofocus class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" placeholder="Nama lengkap" />
                <x-input-error :messages="$errors->get('name')" class="text-sm text-red-600" />
            </div>

            <div class="space-y-2">
                <label for="school" class="block text-sm font-semibold text-slate-900 dark:text-slate-100">Sekolah</label>
                <select id="school" name="sekolah_id" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="space-y-2">
                    <label for="no_hp" class="block text-sm font-semibold text-slate-900 dark:text-slate-100">No HP</label>
                    <input id="no_hp" name="no_hp" value="{{ old('no_hp') }}" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" placeholder="08xxxxxxxxxx" />
                    <x-input-error :messages="$errors->get('no_hp')" class="text-sm text-red-600" />
                </div>

                <div class="space-y-2">
                    <label for="nip" class="block text-sm font-semibold text-slate-900 dark:text-slate-100">NIP</label>
                    <input id="nip" name="nip" value="{{ old('nip') }}" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:border-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900/50" placeholder="NIP guru" />
            ...
            <p class="text-center text-sm text-slate-600 dark:text-slate-400">Sudah disetujui? <a href="{{ route('login', ['role' => 'guru']) }}" class="font-semibold text-blue-600 hover:text-blue-500">Login</a></p>
        </form>
    </div>
</x-guest-layout>
