...
<div class="space-y-6" x-data="{ confirmOpen: false }">
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            />
            <a href="{{ route('siswa.angket.index') }}" class="w-fit rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                Kembali
            </a>
        </div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
                        ...
                        <div class="mt-4 space-y-2">
                            ...
                            <textarea
                                ...
                                required
                                maxlength="500"
                                class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
    ...
    <div
        ...
        aria-modal="true"
    >
        <div x-on:click.outside="confirmOpen = false" class="w-full max-w-md rounded-2xl border border-white bg-white p-6 text-center shadow-2xl shadow-slate-950/20">
            <p class="text-xs font-bold uppercase tracking-[0.24em] text-blue-600">Konfirmasi</p>
            <h2 class="mt-3 text-xl font-bold text-slate-950">Yakin ingin menyimpan jawaban?</h2>
            <p class="mt-3 text-sm leading-6 text-slate-600">Jawaban yang sudah ada akan diperbarui jika Anda mengubahnya.</p>
            <div class="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <button type="button" x-on:click="confirmOpen = false" class="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:border-blue-200 hover:text-blue-700">
                    Batal
                </button>
                <button
                    type="button"
