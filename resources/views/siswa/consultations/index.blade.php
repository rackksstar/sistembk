    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        @if($upcoming->isNotEmpty())
            <div class="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-5">
                <p class="text-xs font-bold uppercase tracking-[0.18em] text-emerald-700">Jadwal mendatang</p>
                <ul class="mt-3 space-y-2">
                    @foreach($upcoming as $item)
                        <li class="flex flex-wrap items-center justify-between gap-2 text-sm">
                            <span class="font-semibold text-slate-900">{{ $item->subject }}</span>
        ...
        <div x-show="formOpen" x-cloak class="mt-6 rounded-3xl border border-blue-100 bg-blue-50/40 p-6">
            ...
            <form action="{{ route('siswa.consultations.store') }}" method="POST" class="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea id="details" name="details" rows="4" required class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-blue-400 focus:ring-2 focus:ring-blue-100" placeholder="Apa yang ingin Anda bahas?">{{ old('details') }}</textarea>
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <form method="GET" class="flex gap-2">
                <select name="status" class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm">
                ...
                <table class="min-w-full divide-y divide-slate-200 text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse($consultations as $consultation)
                            <tr>
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-slate-900">{{ $consultation->subject }}</p>
                                    <p class="mt-1 line-clamp-2 text-xs text-slate-500">{{ $consultation->details }}</p>
                                </td>
                                ...
                                <td class="px-5 py-4">
                                    ...
                                    @if($consultation->status === 'ditolak' && $consultation->rejection_reason)
                                        <p class="mt-2 max-w-xs text-xs text-red-600">{{ $consultation->rejection_reason }}</p>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-8">
                                    <x-empty-state title="Belum ada pengajuan" description="Gunakan tombol di atas untuk mengajukan konseling pertama Anda." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
