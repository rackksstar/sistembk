...
<div class="space-y-6" x-data="{
    ...
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div class="mt-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4 text-sm text-slate-700">
            <p class="font-semibold text-slate-900">{{ $konseling->subject }}</p>
        ...
        <form action="{{ route('siswa.penilaian.store') }}" method="POST" class="mt-6 space-y-8">
            ...
            @foreach($aspects as $aspect)
                <div class="space-y-3">
                    <div>
                        <p class="text-sm font-semibold text-slate-900">{{ $aspect['label'] }}</p>
                        <p class="text-xs text-slate-500">{{ $aspect['hint'] }}</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @endfor
                        <span class="ml-2 text-sm font-semibold text-slate-600" x-text="{{ $aspect['key'] }} ? {{ $aspect['key'] }} + '/5' : 'Pilih skor'"></span>
                    </div>
