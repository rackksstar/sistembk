...
<div class="space-y-6">
    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form method="GET" class="mt-6 grid gap-3 md:grid-cols-[1fr_200px_auto]">
            <input name="search" value="{{ $search }}" placeholder="Cari artikel..." class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm">
        ...
        @forelse($postingan as $item)
            <article class="overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-sm">
                ...
                <div class="space-y-3 p-5">
                    <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">{{ $item->kategori?->name }}</span>
                    <h2 class="text-lg font-semibold text-slate-900">{{ $item->judul }}</h2>
                    <p class="line-clamp-3 text-sm text-slate-600">{{ Str::limit(strip_tags($item->isi), 120) }}</p>
