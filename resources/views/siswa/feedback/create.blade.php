        ...
        <form method="POST" action="{{ route('siswa.feedback.store') }}" class="rounded-3xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-6 shadow-sm">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select name="consultation_request_id" class="mt-1 w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm">
                ...
                <textarea name="message" rows="5" required class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm" placeholder="Apa yang Anda rasakan dari layanan ini?">{{ old('message') }}</textarea>
                <textarea name="suggestion" rows="4" class="w-full rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 px-4 py-3 text-sm" placeholder="Saran untuk layanan BK">{{ old('suggestion') }}</textarea>
            ...
            <div class="mt-5 space-y-3">
                @forelse($feedback as $item)
                    <article class="rounded-2xl bg-slate-50 dark:bg-slate-800/60 p-4 text-sm">
                        <p class="font-semibold text-slate-950 dark:text-white">{{ $item->service_type }} - {{ $item->rating }}/5</p>
                        <p class="mt-1 text-slate-500 dark:text-slate-400">{{ $item->consultation?->subject ?? 'Feedback umum' }}</p>
                        <p class="mt-2 text-slate-600 dark:text-slate-400">{{ $item->message }}</p>
                    </article>
                @empty
